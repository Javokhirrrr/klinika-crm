...
const ALL_MENU_ITEMS = [
    ...
    { to: "/cash-desk", icon: Landmark,  label: "Kassa",     roles: [...ADMIN_ROLES, "cashier", "accountant"] },
        ...
        >
            {({ isActive }) => (
                <>
                    ...
                    {/* Collapsed tooltip */}
                    {collapsed && (
                        <div className="absolute left-full top-1/2 -translate-y-1/2 ml-3 px-3 py-1.5 bg-foreground text-background text-xs font-semibold rounded-lg opacity-0 group-hover:opacity-100 pointer-events-none whitespace-nowrap z-50 transition-opacity duration-100 shadow-xl">
                            {item.label}
...
const AppSidebar = memo(function AppSidebar({ mobileOpen, setMobileOpen, collapsed }) {
    ...
    return (
        <>
            ...
            {/* Sidebar */}
            <aside className={cn(
                "flex flex-col bg-card border-r border-border shadow-sm",
                ...
                <div className={cn(
                    ...
                    <div className="flex items-center gap-3">
                        <div className="w-9 h-9 rounded-xl bg-primary flex items-center justify-center text-white shrink-0 shadow-md">
                            <Activity className="h-4 w-4" />
                        </div>
                        {!collapsed && (
                            <div className="flex flex-col leading-none">
                                <span className="text-xl font-black text-foreground tracking-tight">Zyra</span>
                                <span className="text-[10px] font-bold text-muted-foreground tracking-widest uppercase">CRM</span>
                            </div>
                        )}
                    </div>
                </div>
                ...
                <div className={cn(
                    ...
                    <button
                        ...
                        className={cn(
                            "flex items-center gap-3 text-sm font-semibold text-destructive hover:bg-destructive/10 rounded-xl transition-colors duration-150",
